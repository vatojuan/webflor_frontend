  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/job/admin_offers`, { headers })
      ...
      .then(data => {
        ...
        // Filtrar expiradas
        const valid = raw.filter(o =>
          !o.expirationDate || new Date(o.expirationDate) >= Date.now()
        );
        // Separar admin vs otros
        const mine  = valid.filter(o => o.userId === user.id);
        const other = valid.filter(o => o.userId !== user.id);
        setOffers([...mine, ...other]);
      })
      .catch(err => {
      ...
      <Container sx={{mt:4}}>
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {offers.length===0 && (
                <TableRow><TableCell colSpan={7} align="center">No hay ofertas</TableCell></TableRow>
              )}
              {offers.map(o=>(
                <TableRow key={o.id} sx={{ backgroundColor: o.userId===user.id?"#FFF9C4":"inherit" }}>
                  <TableCell>{o.title}</TableCell>
                  <TableCell>{o.description}</TableCell>
                  <TableCell>{o.requirements}</TableCell>
                  <TableCell>{ o.expirationDate
                  ...
                  <TableCell>
                    <Button size="small" variant="outlined" sx={{mr:1}} onClick={()=>handleEdit(o)}>Editar</Button>
      ...
      <Dialog open={openEditDialog} onClose={()=>setOpenEditDialog(false)} fullWidth>
        ...
        <DialogContent>
          <Box sx={{display:"flex",flexDirection:"column",gap:2,mt:1}}>
            ...
            />
            <TextField label="Descripción" fullWidth multiline rows={4}
              value={selectedOffer?.description||""}
              onChange={e=>handleEditChange("description",e.target.value)}
            />
            <TextField label="Requisitos" fullWidth multiline rows={3}
              value={selectedOffer?.requirements||""}
              onChange={e=>handleEditChange("requirements",e.target.value)}
            />
            <TextField label="Fecha Expiración" type="date" fullWidth InputLabelProps={{shrink:true}}
              value={selectedOffer?.expirationDate?.split("T")[0]||""}
              onChange={e=>handleEditChange("expirationDate",e.target.value)}
            />
            <TextField select label="Etiqueta" fullWidth SelectProps={{native:true}}
            ...
            <TextField select label="Fuente" fullWidth SelectProps={{native:true}}
